...
const logger = createLogger('order:repository');
...
export class FirestoreOrderRepository implements IOrderRepository {
  async findById(id: string): Promise<Order | null> {
    try {
      ...
      throw new RepositoryError('Failed to find order by ID', 'findById', COLLECTION, { id, error: err });
    }
  }
  ...
  async findByOrderNumber(orderNumber: string): Promise<Order | null> {
    try {
      ...
      return docToOrder(results[0]!.id, results[0]!);
    } catch (err) {
      throw new RepositoryError('Failed to find order by number', 'findByOrderNumber', COLLECTION, { orderNumber, error: err });
    }
  }
  ...
  async findByIdempotencyKey(key: string): Promise<Order | null> {
    try {
      ...
      throw new RepositoryError('Failed to find order by idempotency key', 'findByIdempotencyKey', COLLECTION, { key, error: err });
    }
  }
  ...
  async findByUserId(userId: string): Promise<Order[]> {
    try {
      ...
      throw new RepositoryError('Failed to find orders by user', 'findByUserId', COLLECTION, { userId, error: err });
    }
  }
  ...
  async findAll(query: OrderQuery): Promise<PaginatedResult<Order>> {
    try {
      ...
      if (query.orderNumber) constraints.push({ field: 'orderNumber', operator: '==', value: query.orderNumber });
      if (query.status) {
      ...
      if (query.dateFrom) constraints.push({ field: 'createdAt', operator: '>=', value: query.dateFrom });
      if (query.dateTo) constraints.push({ field: 'createdAt', operator: '<=', value: query.dateTo });
      ...
      throw new RepositoryError('Failed to query orders', 'findAll', COLLECTION, { query, error: err });
    }
  }
  ...
  async findByStatus(status: OrderStatus): Promise<Order[]> {
    try {
      ...
      throw new RepositoryError('Failed to find orders by status', 'findByStatus', COLLECTION, { status, error: err });
    }
  }
  ...
  async count(query?: Partial<OrderQuery>): Promise<number> {
    try {
      ...
      throw new RepositoryError('Failed to count orders', 'count', COLLECTION, { query, error: err });
    }
  }
  ...
  async create(data: Order): Promise<Order> {
    try {
      ...
      throw new RepositoryError('Failed to create order', 'create', COLLECTION, { orderNumber: data.orderNumber, error: err });
    }
  }
  ...
  async updateStatus(id: string, status: OrderStatus, changedBy: string, note?: string): Promise<Order> {
    try {
      ...
      throw new RepositoryError('Failed to update order status', 'updateStatus', COLLECTION, { id, status, error: err });
    }
  }
  ...
  async appendTimeline(id: string, entry: OrderTimelineEntry): Promise<Order> {
    try {
      ...
      if (err instanceof NotFoundError) throw err;
      throw new RepositoryError('Failed to append order timeline', 'appendTimeline', COLLECTION, { id, error: err });
    }
  }
  ...
  async updatePayment(id: string, payment: PaymentSummary): Promise<Order> {
    try {
      ...
      if (err instanceof NotFoundError) throw err;
      throw new RepositoryError('Failed to update order payment', 'updatePayment', COLLECTION, { id, error: err });
    }
  }
