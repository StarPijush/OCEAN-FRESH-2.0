import type { Cart, CartItem, CartStatus, CartSource, CartQuery, AddToCartInput } from '@oceanfresh/shared';
...
export interface ICartRepository {
  ...
  create(data: { userId: string | null; sessionId: string | null; source: CartSource }): Promise<Cart>;
  addItem(cartId: string, item: CartItem): Promise<Cart>;
  updateItem(cartId: string, itemId: string, quantity: number, subtotal: { amount: number; currency: string }): Promise<Cart>;
