...
export const CartStateMachine = {
  canTransition(from: CartStatus, to: CartStatus): boolean {
    return VALID_TRANSITIONS[from]?.includes(to) ?? false;
  },

  transition(from: CartStatus, to: CartStatus): void {
    if (!CartStateMachine.canTransition(from, to)) {
      throw new IllegalCartStateTransitionError(from, to);
    }
  },

  isTerminal(status: CartStatus): boolean {
    return [CartStatus.CHECKED_OUT, CartStatus.ARCHIVED, CartStatus.EXPIRED].includes(status);
  },

  isActive(status: CartStatus): boolean {
    return [
      CartStatus.ACTIVE,
      CartStatus.VALIDATING,
      CartStatus.READY_FOR_CHECKOUT,
      CartStatus.CHECKOUT_STARTED,
    ].includes(status);
  },

  getValidTransitions(from: CartStatus): CartStatus[] {
