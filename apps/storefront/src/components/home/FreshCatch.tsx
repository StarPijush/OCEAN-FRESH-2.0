...
export function FreshCatch() {
  ...
  return (
    <section className="section">
      ...
      <h2 className="section-title-lg reveal">Today's<br />Fresh Catch</h2>
      ...
      <div id="fresh-catch-list" style={{ marginTop: '8px' }}>
        {products.map((p) => (
          <div className="catch-row-item reveal" key={p.id}>
            <div>
              <div className="catch-fish-name">{p.emoji} {p.name}</div>
              <div className="catch-fish-sub">{p.sub}</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div className="catch-price">{'\u20B9'}{p.price}</div>
              <div style={{ fontSize: '0.6rem', color: 'var(--muted)', marginTop: '2px' }}>per kg</div>
            </div>
          </div>
        ))}
      </div>
      <div style={{ marginTop: '24px' }} className="reveal">
        <button className="btn btn-outline-dark" onClick={() => navigate('/products')}>Order Now</button>
      </div>
    </section>
  );
}
