    ...
    <div className="modal-overlay show" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ marginBottom: '14px' }}>
          <div style={{ fontSize: '.58rem', letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '3px' }}>Order ID</div>
          <div style={{ fontSize: '.9rem', fontWeight: 600, color: 'var(--aqua)' }}>{order.id}</div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '14px' }}>
          <div>
            <div style={{ fontSize: '.58rem', letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '3px' }}>Customer</div>
            <div style={{ fontSize: '.85rem', color: 'var(--cream)' }}>{order.name}</div>
            <div style={{ fontSize: '.72rem', color: 'var(--muted2)' }}>{order.phone}</div>
          </div>
          <div>
            <div style={{ fontSize: '.58rem', letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '3px' }}>Date & Time</div>
        ...
        <div style={{ marginBottom: '14px' }}>
          <div style={{ fontSize: '.58rem', letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '6px' }}>Delivery Address</div>
          <div style={{ fontSize: '.82rem', color: 'var(--cream)', lineHeight: 1.5 }}>{order.address}</div>
        </div>

        <div style={{ height: '1px', background: 'var(--border)', margin: '14px 0' }} />

        <div style={{ fontSize: '.58rem', letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '8px' }}>Items</div>
        {(order.items ?? []).map((it, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
            <div>
              <div style={{ fontSize: '.85rem', color: 'var(--cream)' }}>{it.name}</div>
              <div style={{ fontSize: '.7rem', color: 'var(--muted2)' }}>{it.qty}kg × {fmt(it.price)}</div>
            </div>
            <div style={{ fontWeight: 600, color: 'var(--aqua)' }}>{fmt(it.sub)}</div>
          </div>
        ))}

        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', fontWeight: 700, fontSize: '.95rem' }}>
          <span>Total</span>
        ...
        <div style={{ fontSize: '.58rem', letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '8px' }}>Update Status</div>
