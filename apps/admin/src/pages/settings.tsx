...
export function SettingsPage() {
  ...
  const saveProfile = async () => {
    if (!admin.name.trim()) { toast('Name cannot be empty', 'error'); return; }
    if (admin.mobile.length < 10) { toast('Enter a valid mobile number', 'error'); return; }
    await authRepository.updateAdmin({ name: admin.name.trim(), mobile: admin.mobile.trim() });
    toast('Profile updated', 'success');
  };

  const savePassword = async () => {
    if (curPass !== admin.password) { toast('Current password is incorrect', 'error'); return; }
    if (newPass1.length < 6) { toast('New password must be at least 6 characters', 'error'); return; }
    if (newPass1 !== newPass2) { toast('Passwords do not match', 'error'); return; }
  ...
  const saveDelivery = async () => {
    if (delivery.amount < 0) { toast('Charge cannot be negative', 'error'); return; }
      ...
      <div className="settings-card">
        ...
        <div className="settings-card-body">
          <div className="form-grp">
            ...
            <input className="form-inp" type="text" placeholder="Shop Owner"
          ...
          <div className="form-grp">
            ...
            <div className="form-inp-prefix">
              ...
              <input className="form-inp" type="tel" placeholder="9876543210" maxLength={10}
                value={admin.mobile} onChange={e => setAdmin(a => ({ ...a, mobile: e.target.value }))} />
            </div>
          </div>
      ...
      <div className="settings-card">
        ...
        <div className="settings-card-body">
          <div className="form-grp">
            ...
            <input id="settings-cur-pass" className="form-inp" type="password" placeholder="Current password"
              value={curPass} onChange={e => setCurPass(e.target.value)} />
          </div>
          <div className="form-grp">
            <label className="form-lbl">New Password</label>
            <input id="settings-new-pass" className="form-inp" type="password" placeholder="New password"
              value={newPass1} onChange={e => setNewPass1(e.target.value)} />
          </div>
          <div className="form-grp">
            <label className="form-lbl">Confirm Password</label>
            <input id="settings-confirm-pass" className="form-inp" type="password" placeholder="Confirm password"
      ...
      <div className="settings-card">
        ...
        <div className="settings-card-body">
          <div className="form-grp">
            ...
            <div className="form-inp-prefix">
              <span className="prefix">+91</span>
              <input id="settings-wa-val" className="form-inp" type="tel" placeholder="9876543210" maxLength={10}
                value={wa.replace('91', '')} onChange={e => setWa('91' + e.target.value.replace(/\D/g, '').slice(0, 10))} />
            </div>
          </div>
          <button className="btn btn-primary" onClick={saveWA}>Save WhatsApp</button>
        </div>
      </div>
      ...
      <div className="settings-card">
        ...
        <div className="settings-card-body">
          <div className="form-grp">
            ...
            <input id="settings-delivery-charge" className="form-inp" type="number" min="0" step="10" placeholder="0"
              value={delivery.amount} onChange={e => setDelivery(d => ({ ...d, amount: parseFloat(e.target.value) || 0 }))} />
          </div>
          <div className="form-grp">
            <label className="form-lbl">Free above (₹)</label>
            <input id="settings-delivery-free-above" className="form-inp" type="number" min="0" step="50" placeholder="0"
              value={delivery.freeAbove} onChange={e => setDelivery(d => ({ ...d, freeAbove: parseFloat(e.target.value) || 0 }))} />
          </div>
          <button className="btn btn-primary" onClick={saveDelivery}>Save Delivery</button>
        </div>
      </div>
    </div>
  );
